...
export const releaseSeatsAndDeleteBookings = inngest.createFunction(
  { id: "release-seats-expires-booking" }, //uniquely identifies your app in Inngest,groups all workflows under one app,is used in the Inngest dashboard
...
export const sendShowReminders = inngest.createFunction(
  {id:'send-show-reminders'}, // Unique ID for this Inngest function
  ...
  async ({step}) => {
   ...
   const now = new Date(); // Current time
    ...
    const {reminderTasks,showCount} = await step.run(
      'prepare-reminder-tasks',  // Step name (shows in Inngest dashboard)
      async () => {
               // Query shows within the time window
                const shows = await ShowModel.find({
                  ...
                  showDateTime:{
                    ...
                    $lte:windowEnd }     // Shows starting within 8 hours
                }).populate([
                  {
                    path:'movieRef',
                    model:'Movie',
...
export const  sendNewShowNotifications = inngest.createFunction(
  ...
  {event:'new-show-added'},  // every 12 hours 
  async ({event,step}) => {
...
export const sendPersonalShowReminder = inngest.createFunction(
  { id: "send-scheduled-show-reminder" },
  { event: "app.scheduleShowReminder" },
  async ({ event, step }) => {
    try {
    ...
    if (mongoose.connection.readyState !== 1) {
      ...
      await step.run("send-reminder-email", async () => {
        ...
        const hoursUntilShow = Math.round(
