import MovieModel, { type IMovie } from "../models/movie.schema.js";
...
export function getBestPlayableVideo(videos: Video[]) {
  if (!videos?.length) return null;

  const ytOfficial = videos.filter(
    v => v.site === "YouTube" && v.official === true
  );

  if (!ytOfficial.length) return null;
  ...
  for (const type of PRIORITY) {
    ...
    const exact = candidates.find(v =>
      v.name.toLowerCase().includes(`official ${type.toLowerCase()}`)
    );
...
export async function IS_MOVIE_EIXST_SERVICE(movieId:string):Promise< IMovie | null>{
